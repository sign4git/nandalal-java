package com.example.jobspringbootrest.repository;
...
public class JobRepository {
    ...
    public JobPost findById(int postId) {
        return jobs.stream().
                filter(jobs->jobs.getPostId()==postId).
                findFirst().orElse(new JobPost());
    }

    public JobPost updateJob(JobPost jobPost, int postId) {
        Optional<JobPost> jobToBeUpdated=jobs.stream()
                .filter(jobs->jobs.getPostId()==postId)
                .findFirst();
        if(jobToBeUpdated.isPresent()){
            JobPost jobToUpdate=jobToBeUpdated.get();
            jobToUpdate.setPostDesc(jobPost.getPostDesc());
            jobToUpdate.setReqExperience(jobPost.getReqExperience());
            jobToUpdate.setPostProfile(jobPost.getPostProfile());
            jobToUpdate.setPostTechStack(jobPost.getPostTechStack());
